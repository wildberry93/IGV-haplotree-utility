...
public class manipulateVCFregion{
	...
	public void getROI(RegionOfInterest reg){
		...
		SAMSequenceDictionary refDict = vcfData.getFileHeader().getSequenceDictionary(); //sequence dict for saving vcf 
		List<VariantContext> IterCopy = new ArrayList<VariantContext>(); //copy iterator to list to refer to the curr element
		VCFHeader vcfHeader = vcfData.getFileHeader();
		
		//System.out.println(refDict);
		...
		try{
			saveRegionFilePro(vcfIterRegion, refDict, vcfHeader);
		}catch(Exception e){
	...
	public void saveRegionFilePro(ArrayList<VariantContext> vcfIterRegion, SAMSequenceDictionary refDict, VCFHeader vcfHeader){
		//Build WriterBuilder
